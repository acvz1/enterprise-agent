...
HTTP 上传文件
  ...
  -> Service 保存文件并创建 uploadId / PENDING 任务
  -> Worker 通过 taskExecutor 后台处理
  -> 前端根据 uploadId 查询最终状态
```

当前只需要记住的分层：

```text
Controller：接收 HTTP 输入并返回响应
Service：执行业务判断和流程编排
Repository：读写数据库
Entity / DTO：承载数据库数据或接口数据
```
...
阅读时一次只跟一个接口，不再同时展开 Spring、异步、RAG 和所有注解。
...
```text
普通方法调用：当前线程必须等待方法返回
提交给 taskExecutor：请求线程可以先返回，工作线程继续处理
```
...
5 个核心线程
  -> 忙时最多排队 100 个任务
  -> 队列满后最多扩到 10 个线程
  -> 仍然满载时由 AbortPolicy 拒绝任务
```

拿到 `uploadId` 只表示任务已受理；真正成功与否由 `UploadProgress` 的最终状态决定。

<details>
<summary>展开旧版详细推导（当前非必读）</summary>
...
```text
DocumentProcessingService 调用注入的 Worker
  -> 实际先进入 Spring 创建的 Worker 代理
  -> 代理把 uploadId 提交给 taskExecutor
  -> file-processing-* 线程执行真实 Worker 方法
```

`@Async` 标记“这个方法需要异步执行”，真正切换线程的是代理提交线程池。

当前项目把受理任务和后台处理拆成两个 Bean，是为了让调用从外部进入 Worker 代理；同一个对象内部使用 `this` 调用自己的 `@Async` 方法，会绕过这一步。

<details>
<summary>展开旧版 Spring 代理详细解释（当前非必读）</summary>
...
| 阶段 | 代表注解 | 当前作用 |
| --- | --- | --- |
| 应用启动 | `@Service`、`@Configuration`、`@Bean` | 创建并连接项目需要的对象 |
| HTTP 请求 | `@RestController`、`@PostMapping`、`@RequestParam` | 注册接口并接收请求数据 |
| 方法执行前后 | `@PreAuthorize`、`@Async`、`@Transactional` | 增加权限、异步或事务行为 |

需要哪个注解时再看对应小节，不把整张速查表当成当前学习任务。

<details>
<summary>展开完整注解速查（按需阅读）</summary>
