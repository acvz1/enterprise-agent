...
- [021. 数据同步与 RRF：从双路候选到融合 Top K](#021-数据同步与-rrf从双路候选到融合-top-k)
...
- [022. RRF Top K 回查 MySQL：让模型使用 chunk 并返回引用](#022-rrf-top-k-回查-mysql让模型使用-chunk-并返回引用)

***Agent 与权限主线***

- [023. Agent 工具决策与检索前后权限校验](#023-agent-工具决策与检索前后权限校验)

***检索评测主线***

- [024. 固定语料检索评测：Hit@K、Recall@K、延迟与 SQL 次数](#024-固定语料检索评测hitkrecallk延迟与-sql-次数)
...
截至 2026-07-26，六天核心计划的第 1～5 天已经真实验收完成，总进度约为 83.3%。当前断点进入第 6 天：稳定 Demo、架构图、README、STAR 简历描述和面试追问。
...
这是一个 `Vue 3 + Spring Boot + MySQL + Redis Stack + Elasticsearch + LangChain4j` 实现的企业知识库 RAG / Agent 系统：用户把企业文档导入知识库，系统解析并切成 chunk，建立向量与 BM25 索引，问答时通过混合检索取得证据；普通 RAG 直接使用证据回答，Agent 接口则由模型决定是否调用知识库工具。

当前代码同时保留固定 RAG 问答和最小 Agent 问答。Agent 已能根据问题选择是否调用 `searchKnowledgeBase`，观察混合检索结果后生成回答，并返回工具名与引用；它不是支持任意多步循环和大量工具的通用 Agent。
...
二开后的检索主线已经改为 Redis 向量召回与 Elasticsearch BM25 召回统一输出 `RetrievalCandidate`，再通过 RRF 融合、Top K 一次回查 MySQL，组装包含正文和引用来源的 `RetrievalHit`。Redis `TextSegment` 已携带 `documentId` 与 `chunkIndex`，候选阶段不再按正文逐条反查 MySQL。
...
### 021. 数据同步与 RRF：从双路候选到融合 Top K

**需求**

Redis 向量检索和 Elasticsearch BM25 已经能独立返回 `RetrievalCandidate`，但两个列表可能包含同一个 chunk，原始分数又不在同一个量纲中。系统需要先保证文档变更时两套索引同步，再用统一身份识别同一 chunk，并根据两路排名得到稳定的 Top K。

**代码位置**

- [DocumentChunkService.java](D:/Project/enterprise-agent/src/main/java/com/kb/demo/service/DocumentChunkService.java)：分块重建时同步更新 Redis 和 Elasticsearch。
- [ElasticsearchSearchService.java](D:/Project/enterprise-agent/src/main/java/com/kb/demo/service/ElasticsearchSearchService.java)：按 `documentId` 删除旧 chunk，批量写入当前 chunk，并统一 refresh。
- [RrfFusionService.java](D:/Project/enterprise-agent/src/main/java/com/kb/demo/service/RrfFusionService.java)：识别同一 chunk、累加 RRF 分数、合并来源并截取 Top K。
- [HybridRetrievalService.java](D:/Project/enterprise-agent/src/main/java/com/kb/demo/service/HybridRetrievalService.java)：编排 Redis、Elasticsearch 和 RRF。
...
Document 当前正文
  -> 开始重建该文档的分块
  -> 按 documentId 删除 Elasticsearch 全部旧 chunk
  -> 重新切分正文
  -> MySQL 保存当前 DocumentChunk
  -> Redis 保存向量、TextSegment 和 documentId/chunkIndex
  -> Elasticsearch 批量写入 documentId/chunkIndex/content
  -> 所有 chunk 写完后统一 refresh
```

删除旧数据不是为了取得正文，而是为了保证更新后的文档不会同时留下“旧 chunk + 新 chunk”。正文只在随后重新切分并写入当前版本时使用。

**RRF 融合链路**

```text
Redis List<RetrievalCandidate>
  + Elasticsearch List<RetrievalCandidate>
  -> documentId + chunkIndex 组成同一 chunk 的业务键
  -> 每路候选贡献 1 / (60 + rank)
  -> 同一 chunk 的贡献累加
  -> sources 合并为 Set<RetrievalSource>
  -> fusionScore 降序排列
  -> 截取 Top K
```

`fusionScore` 只表示本次查询中的融合排序强弱，不是相关概率。两路都靠前的 chunk 会同时包含 `REDIS_VECTOR` 和 `ELASTICSEARCH_BM25`，但 RRF 也允许某个结果只来自一路。
...
HybridRetrievalServiceIT：
真实连接 Redis Stack 和 Elasticsearch
同一测试 chunk 的 sources 同时包含两路来源
Tests run: 1，Failures: 0，Errors: 0
```

**结论**

同步保证两套可重建索引与当前文档版本一致；RRF 则在不直接比较两路原始分数的前提下，把重复候选合并成统一、可解释的 Top K。

**面试表达**

> 我没有直接相加 Redis 相似度和 BM25 `_score`，因为二者含义和量纲不同。我先把两路结果转换成统一候选，用 `documentId + chunkIndex` 识别同一 chunk，再根据各自 rank 使用 RRF 累加贡献、合并来源并截取 Top K。该链路通过单元测试和真实 Redis、Elasticsearch 集成测试验证。

---

### 022. RRF Top K 回查 MySQL：让模型使用 chunk 并返回引用

**需求**

原问答链虽然先做向量检索，最终却返回完整 `Document` 并把整篇正文放入 Prompt。真正命中的 chunk 可能被无关章节淹没，接口也无法告诉用户答案具体依据了哪一段。改造目标是：
...
RRF Top K
  ...
  -> 补全文档标题和权威 chunk 正文
  -> 组装 RetrievalHit
  -> 普通与流式 Prompt 使用精确 chunk
  -> 返回答案和 citations
```

**代码位置**

- [DocumentChunkRepository.java](D:/Project/enterprise-agent/src/main/java/com/kb/demo/repository/DocumentChunkRepository.java)：批量查询候选 chunk，并通过 `JOIN FETCH` 同时取得所属 Document。
- [RetrievalResultService.java](D:/Project/enterprise-agent/src/main/java/com/kb/demo/service/RetrievalResultService.java)：把融合候选补全成最终 `RetrievalHit`。
- [HybridRetrievalService.java](D:/Project/enterprise-agent/src/main/java/com/kb/demo/service/HybridRetrievalService.java)：`searchHits()` 串联混合检索和 MySQL 补全。
- [AiService.java](D:/Project/enterprise-agent/src/main/java/com/kb/demo/service/AiService.java)：普通和流式问答使用 chunk 构建 Prompt，并返回引用。
- [RetrievalResultServiceMySqlIT.java](D:/Project/enterprise-agent/src/test/java/com/kb/demo/service/RetrievalResultServiceMySqlIT.java)：使用隔离 MySQL 验证真实 JPQL 和 SQL 次数。

**为什么先 Top K，再查 MySQL**

候选阶段只需要身份、排名、分数和来源。如果 Redis、Elasticsearch 刚召回就逐条查表，查询次数会随候选数量增长。当前链路先用两个索引完成召回和排序，只对最终少量 Top K 查询权威数据：

```text
大量候选：Redis / Elasticsearch 内部处理
最终 Top K：MySQL 一次补全
模型输入：只使用补全后的 RetrievalHit
```

MySQL 是 `Source of Truth`；Redis 和 Elasticsearch 是可重建索引。索引负责找候选，最终标题和正文仍以 MySQL 当前数据为准。
...
它可能查出两个集合的交叉组合。例如候选是 `(1,0)` 和 `(2,1)`，查询结果中还可能出现 `(1,1)`。因此 `RetrievalResultService` 建立：

```text
key = documentId + "_" + chunkIndex
```

再按照原候选列表逐个取回精确组合。这样既过滤交叉结果，也保持 RRF 排序；索引中存在但 MySQL 已不存在的陈旧候选会被跳过。

`JOIN FETCH dc.document` 让每个 `DocumentChunk` 在同一条查询中带回所属 `Document`，后续读取标题不会再触发额外 SQL。
...
`AiService` 用标题、分块位置和正文构建 Prompt，不再放入整篇 `Document.content`。普通接口在响应中返回 `citations`；SSE 流式接口把引用放在最后的 `metadata` 事件中，只发送一次，不随每个 token 重复发送。

回答缓存只保存答案，没有保存引用。因此代码先重新执行检索得到当前 `relevantHits`，再读取答案缓存：

```text
检索当前证据
  -> 缓存命中：跳过模型调用，但返回当前 citations
  -> 缓存未命中：调用模型、缓存答案并返回 citations
```

这保留了模型调用缓存的主要收益，同时避免缓存答案有引用、缓存命中却没有引用的接口不一致。

**真实验证**

1. `RetrievalResultServiceTest` 故意让 Repository 乱序返回，并夹带交叉组合和陈旧候选：
...
2. `RetrievalResultServiceMySqlIT` 使用真实 MySQL：

```text
保存 1 篇 Document 和 2 个 DocumentChunk
清空 JPA 一级缓存
调用 assembleHits()
实际 SQL 为一次 document_chunks JOIN documents
Tests run: 1，Failures: 0，Errors: 0
```
...
```text
HTTP 200
DeepSeek 根据知识库 chunk 回答
citations 返回 3 条
每条包含标题、chunkIndex、正文、fusionScore、sources
```
...
```text
message：连续回答片段
metadata：fromCache、model、非空 citations
缓存命中时 fromCache = true，引用仍然存在
```

本次业务问答样本的三个引用都只包含 `REDIS_VECTOR`，只能证明 RRF 支持单路候选；双路同时命中已经由 `HybridRetrievalServiceIT` 独立验证。后续 Demo 仍需准备一个稳定返回双路来源的样本。

**真实错误与根因**

```text
403 Forbidden
  -> 旧 JWT 已超过 application.yml 配置的 24 小时
  -> 重新登录并替换 accessToken 后恢复

500 Internal Server Error
  -> 检索依赖健康，但没有配置 DEEPSEEK_API_KEY
  -> 在被 .gitignore 忽略的 .env 中配置密钥并重启后恢复
```
...
```text
功能改造
  -> Document 级单路检索变成 chunk 级混合检索和引用返回

结构重构
  -> AiService 从字段注入统一为构造器注入
  -> 删除不再使用的 DocumentRepository、Document、VectorSearchService
  -> buildEnhancedPrompt 参数从 List<Document> 改为 List<RetrievalHit>
```

从 `Document` 改成 `RetrievalHit` 时，局部变量、Prompt 参数、遍历代码、统计数量和依赖注入都需要同步修改。这是上游数据类型贯穿多个消费者产生的 `Ripple Effect`（连锁修改），不是“重构”一词本身的定义。

**结论**

系统已经从“向量命中后返回整篇文档”改为“Redis 与 Elasticsearch 召回、RRF 融合、Top K 一次回查 MySQL、精确 chunk 进入 Prompt，并在普通和流式接口中返回可核验引用”。这条主链已经通过单元测试、真实 MySQL、真实双路检索和 Postman HTTP/SSE 四层验证。

**面试表达**

> 原系统虽然使用向量检索，但最终把完整文档交给模型，命中片段可能被无关正文淹没，也无法返回可核验引用。我把 Redis 向量召回和 Elasticsearch BM25 统一为轻量候选，通过 RRF 融合后只对 Top K 一次批量回查 MySQL，组装包含标题、正文、融合分数和来源的 RetrievalHit。普通与 SSE 问答都使用精确 chunk 构建 Prompt，并返回 citations；真实 MySQL 测试确认补全只执行一条查询，Postman 也验证了缓存命中时仍能返回当前引用。

---

### 023. Agent 工具决策与检索前后权限校验

**需求**

固定 RAG 无论用户问什么，后端都会先执行检索再调用模型。企业知识库 Agent 需要让模型根据问题决定是否调用知识库工具，同时不能因为增加工具调用而绕过原有权限边界：
...
- [AgentController.java](D:/Project/enterprise-agent/src/main/java/com/kb/demo/controller/AgentController.java)：Agent HTTP 入口，通过 `@PreAuthorize` 检查 `qa:ask`。
- [KnowledgeAgent.java](D:/Project/enterprise-agent/src/main/java/com/kb/demo/agent/KnowledgeAgent.java)：定义模型角色、工具调用规则和空结果回答规则。
- [KnowledgeBaseTool.java](D:/Project/enterprise-agent/src/main/java/com/kb/demo/agent/KnowledgeBaseTool.java)：把混合检索暴露为 LangChain4j 工具，并在返回前检查 `document:read`。
- [KnowledgeAgentService.java](D:/Project/enterprise-agent/src/main/java/com/kb/demo/service/KnowledgeAgentService.java)：创建 Agent、执行对话、读取工具执行记录并提取 citations。
- [AgentResponse.java](D:/Project/enterprise-agent/src/main/java/com/kb/demo/dto/AgentResponse.java)：返回回答、模型、是否调用工具、工具名和引用。
- [JwtAuthenticationFilter.java](D:/Project/enterprise-agent/src/main/java/com/kb/demo/security/JwtAuthenticationFilter.java)：验证 JWT 后把当前用户及权限写入 `SecurityContextHolder`。
- [RolePermissionInitializer.java](D:/Project/enterprise-agent/src/main/java/com/kb/demo/init/RolePermissionInitializer.java)：初始化角色与权限；GUEST 当前默认没有业务权限。

**Agent 调用链**

```text
POST /api/ai/agent/ask
  -> AgentController 读取 question、model
  -> @PreAuthorize("hasAuthority('qa:ask')")
  -> KnowledgeAgentService 创建 ChatLanguageModel
  -> AiServices.builder(KnowledgeAgent.class)
       .chatLanguageModel(model)
       .tools(knowledgeBaseTool)
       .build()
  -> agent.chat(question)
  -> 模型读取 @SystemMessage 和工具描述
  -> 决定是否调用 searchKnowledgeBase
```

`AiServices.builder(...)` 创建实现 `KnowledgeAgent` 接口的代理对象。调用 `agent.chat(question)` 时，不是执行手写的接口实现，而是由 LangChain4j 组织模型请求、接收工具调用、执行 Java 工具方法、把工具结果交回模型，再取得最终回答。

`KnowledgeBaseTool` 使用：

```java
@Tool("搜索企业知识库……")
public List<RetrievalHit> searchKnowledgeBase(
        @P("需要在企业知识库中检索的完整问题")
        String query) {
}
```

- `@Tool` 的说明会进入模型可见的工具定义，告诉模型这个方法解决什么问题。
- `@P` 描述工具参数，让模型知道 `query` 应传入完整检索问题。
- 返回的 `List<RetrievalHit>` 会被序列化为工具结果，模型看到的是包含标题、正文、融合分数和来源的结构化证据。

**怎样证明模型真的调用了工具**

`KnowledgeAgent.chat()` 返回 `Result<String>`，而不是只有回答字符串。两部分信息的用途不同：
...
`KnowledgeAgentService` 根据 `toolExecutions()` 生成：
...
只有回答内容相似，不能证明工具被调用；`toolUsed=true`、工具名为 `searchKnowledgeBase` 且 citations 非空，才是这条运行路径的直接证据。

工具结果是 JSON 字符串，`ObjectMapper.readValue(...)` 将其还原成 `RetrievalHit[]`，再加入响应的 citations。这一步只提取已经发生的工具执行结果，不重新检索一次。
...
```text
JWT Filter 验证 token 并加载用户权限
  -> Spring 在 Controller 方法执行前检查 qa:ask
  -> 有权限才进入 Agent
  -> 无权限直接返回 HTTP 403
```

不带 Token 得到 403 只能证明“必须登录”。本轮使用真实临时 GUEST 身份验证了更强的结论：

```text
登录成功，HTTP 200
  -> JWT 有效
  -> GUEST 没有 qa:ask
  -> 请求 Agent 返回 HTTP 403
```

这证明拒绝原因是授权失败，而不是没有完成身份认证。

GUEST 原来同时拥有 `document:read` 与 `qa:ask`。当前系统的 `document:read` 是全局权限，还不能区分公开与内部文档，因此给 GUEST 读取权限等于放开全部文档。按照“默认拒绝、最小权限”，GUEST 改为无业务权限：
...
`createRoleIfNotExists()` 只在角色不存在时执行创建逻辑，修改初始化代码不会自动清除数据库中既有 GUEST 的权限关联。本轮先查询 `role_permissions` 确认旧的两条关系，再一次性删除并回查；没有把初始化器改成每次启动覆盖数据库权限，避免重启抹掉管理员以后的配置。

**检索后权限**

`qa:ask` 只表示允许使用问答接口，不等于允许读取文档。工具取得 `RetrievalHit` 后，通过：
...
JwtAuthenticationFilter
  -> 创建包含 userDetails.getAuthorities() 的 Authentication
  -> 写入 SecurityContextHolder
  -> KnowledgeBaseTool 读取同一请求的 Authentication
  -> getAuthorities() 取得权限集合
```

如果没有 `document:read`，工具返回空列表，不把 `RetrievalHit.content` 暴露给模型。不能把“无权限”伪装成一条 `RetrievalHit`，否则提示文本会污染证据和 citations。

当前工具返回类型只有列表，无法区分“没有相关结果”和“结果因权限被过滤”。最小安全提示统一为：

```text
未找到当前账号可访问的知识库内容
```

它既给用户反馈，也不泄露系统中是否存在某篇无权查看的文档。若以后要明确区分原因，应把工具结果改成包含 `authorized`、`message` 和 `hits` 的结构化 DTO，而不是伪造检索命中。
...
这两个请求共同证明工具调用是模型的条件选择，不是后端固定执行。
...
```text
临时 QA-only 身份：只有 qa:ask，没有 document:read
HTTP 200
toolUsed = true
toolNames = [searchKnowledgeBase]
citations = 0
answer = 未找到当前账号可访问的知识库内容
```
...
```text
qa:ask       -> 能否使用 Agent
document:read -> 工具结果能否把文档正文交给模型
```

它还不是逐文档 ACL，不能表达部门、租户、文档密级、公开范围或单篇授权。面试时可以说明“权限检查放在模型看到证据之前”，但不能宣称已经完成生产级企业文档权限系统。

**结论**

系统已经从固定 RAG 增加了一条可验证的 Agent 路径：模型根据问题决定是否调用混合检索工具，后端通过实际工具执行记录返回引用；`qa:ask` 在请求进入前控制 Agent 使用权，`document:read` 在检索结果返回模型前阻止正文泄露。

**面试表达**

> 我没有把 Agent 理解成多写一层 Service，而是让模型真正拥有是否调用知识库工具的选择权。我用 LangChain4j AiServices 把现有混合检索封装成 `searchKnowledgeBase`，并通过 `Result.toolExecutions()` 返回工具名与引用，避免仅凭回答文本猜测是否调用工具。安全上采用双层校验：Controller 先检查 `qa:ask`，工具取得 RetrievalHit 后再从 SecurityContext 检查 `document:read`，无权时在正文进入模型前过滤为空。知识库问题、普通问候、GUEST 403 和 QA-only 空引用四条真实请求分别验证了工具选择与权限边界。

---

### 024. 固定语料检索评测：Hit@K、Recall@K、延迟与 SQL 次数

**需求**

Redis、Elasticsearch 和 RRF 链路能够运行，只能证明功能打通，不能回答“检索效果怎样、哪一路更好、混合检索是否值得、Top K 回查是否真的只有一条 SQL”。Day5 因此使用同一批语料和标准答案比较三种策略，而不是各挑一个有利样本。

**代码位置**

- [RetrievalEvaluationIT.java](D:/Project/enterprise-agent/src/test/java/com/kb/demo/service/RetrievalEvaluationIT.java)：准备固定语料、执行三路检索并计算指标。
- [RetrievalResultServiceMySqlIT.java](D:/Project/enterprise-agent/src/test/java/com/kb/demo/service/RetrievalResultServiceMySqlIT.java)：用真实 MySQL 验证 Top K 补全的 SQL 次数。
- [05-retrieval-evaluation-results.md](D:/Project/enterprise-agent/docs/05-retrieval-evaluation-results.md)：保存评测设计、实测表格、分析和边界。
...
每个问题都提前标注正确的 `documentId + chunkIndex`，例如：

```text
问题：报销时限和大额费用审批规则是什么
标准答案：950002_0、950002_1
```

三种检索策略最终都转成相同的 chunk key，因此比较的是：
...
测试数据使用 `950001`～`950004` 四个专用文档 ID。`setUp()` 先删除同 ID 旧数据再写入，`tearDown()` 只删除这些测试数据，不清空开发环境中的全部索引。实测结束后 Redis 和 Elasticsearch 中的评测数据剩余数量都为 0。
...
```text
Hit@3
  -> 前三条只要至少有一个正确 chunk，该问题就算命中
  -> 所有命中问题数 / 全部问题数

Recall@3
  -> 前三条命中的正确 chunk 数 / 该问题全部正确 chunk 数
```

单答案问题中，找到正确 chunk 时两者通常都为 1。多答案问题更能体现区别：
...
所以 Hit@3 回答“前三条里有没有可用证据”，Recall@3 回答“需要的证据找全了多少”。

**代码执行链**

```text
@BeforeEach
  -> 连接真实 Redis 与 Elasticsearch
  -> 创建索引
  -> 清理旧评测数据
  -> 把 8 个 chunk 同时写入两套索引
...
evaluateStrategy()
  -> 逐个执行 15 个问题
  -> System.nanoTime() 记录单次检索耗时
  -> isHitAtK() 判断是否至少命中
  -> recallAtK() 计算正确分块召回比例
  -> 汇总 Hit@3、Recall@3、平均延迟和 P95

@AfterEach
  -> 只清理评测专用数据
  -> 关闭 Redis 与 Elasticsearch 客户端
```

`SearchOperation` 是只有一个 `search(query)` 方法的函数式接口。它让 `evaluateStrategy()` 不需要知道内部调用的是 Redis、Elasticsearch 还是 RRF；三路只要都能把查询转换成 `List<String>` chunk key，就可以复用同一套指标逻辑。

平均延迟反映全部请求的整体耗时；P95 先把 15 个耗时排序，再取 95% 位置的值，用来观察较慢请求。这里是本机小样本单次测量，只能用于当前三条实现的相对比较，不能作为生产 SLA。
...
`prepareStatementCount = 1` 的证明链是：

```text
RRF Top K
  -> Repository 用 JOIN FETCH 一次查询 chunk 与 Document
  -> 内存按 documentId + chunkIndex 精确匹配
  -> 组装 RetrievalHit
```

这证明最终补全没有变成“每个候选再查一次”的 N+1 查询。

**怎样理解结果**

本次中文企业制度小语料中，BM25 最好，因为“年假、病假、报销、VPN、数据库变更”等关键词区分度很高。不能为了让架构看起来漂亮，就把结果改写成“混合检索一定最好”。

RRF 相比 Redis 向量检索：

```text
Hit@3：0.8000 -> 0.9333
Recall@3：0.7667 -> 0.9333
```

说明 BM25 排名修复了大部分向量误召回；但弱向量候选也可能通过 RRF 把无关结果推入 Top 3，因此混合结果没有超过本次 BM25。RRF 的作用是融合互补信号、降低单路失误风险，不是数学上保证超过每一个单路结果。

混合检索延迟最高的直接代码原因是当前 `HybridRetrievalService` 顺序执行：

```text
Redis 向量检索完成
  -> Elasticsearch BM25 完成
  -> RRF 融合
```

两路还没有并行；`VectorSearchService` 也会在每次查询时重新创建嵌入模型和 Redis store。这两个现象是后续性能优化依据，当前不能写成已经完成的能力。

**结论**

Day5 将“检索能跑”推进到了“检索效果和成本可测”：固定语料让三路结果可公平比较，Hit@3/Recall@3 表达检索质量，平均/P95 表达当前本机耗时，真实 MySQL 统计证明 Top K 只用一条 SQL 补全。评测也诚实暴露了当前 BM25 强于混合检索、双路串行和向量链路初始化开销，而不是只保留有利结果。

**面试表达**

> 我没有用一两个成功问题证明检索效果，而是建立了包含 8 个 chunk、15 个标注问题的固定评测集，用同一套 documentId + chunkIndex 标准答案比较 Redis 向量、Elasticsearch BM25 和 RRF。评测统计 Hit@3、Recall@3、平均延迟和 P95，并通过真实 MySQL 测试确认 Top K 补全只有一条 SQL。当前小规模中文制度语料中 BM25 的 Hit@3 和 Recall@3 都是 1.0，RRF 为 0.9333，优于单独向量检索但没有超过 BM25；我据此定位到向量候选质量、串行双路调用和每次重建客户端是下一步优化方向，而没有宣称混合检索天然更强。
