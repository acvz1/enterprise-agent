...
- 使用 `qa:ask` 和 `document:read` 完成请求入口与混合检索前的两层权限校验。
...
|---|---:|---:|---:|---:|
| Redis 向量检索 | 0.8000 | 0.7667 | 32.698 ms | 50.175 ms |
| Elasticsearch BM25 | 0.9333 | 0.9333 | 33.003 ms | 57.776 ms |
| RRF 混合检索 | 0.9333 | 0.9333 | 50.360 ms | 72.425 ms |

以上为 2026-08-09 在本机 Docker Redis Stack、Elasticsearch 8.10.4 与 15 问固定语料上的一次集成测试结果，不是生产压测结论。当前小规模中文制度语料中，RRF 明显优于单独向量召回，但与 BM25 持平；Top K 的 MySQL 补全实测只执行 1 条 SQL。
...
- 有效 GUEST JWT 请求文档列表、普通 RAG 提问均返回 HTTP 403；请求不会进入检索或模型调用。

## 5. 当前边界

- 权限是全局 `qa:ask + document:read`，不是逐文档 ACL。
- 公开注册只会得到没有业务权限的 GUEST，尚未实现管理员审批或角色授予流程。
- 默认管理员仅用于本地演示；首次部署必须改密并改为外部安全配置，初始化日志不再输出密码。
- MySQL、Redis、Elasticsearch 不是同一个事务；文档新建、正文更新与版本回滚都会触发删除旧索引、重建 chunk 和双路索引，但生产环境仍需 Outbox、重试和版本切换保证最终一致性。
