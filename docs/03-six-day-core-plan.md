# 企业知识库 Agent：六天核心开发计划与验收

> 六天核心开发已经于 2026-07-27 完成。本文件保留目标、技术取舍和逐日验收证据，用于复盘本轮二次开发，不再混入早期 25～38 天的生产级设想。
...
本次小规模中文企业制度语料中 BM25 表现最好；RRF 相比单独向量检索提高了命中与召回，但受到较弱向量排序影响，没有超过 BM25。混合链路当前串行执行，因此延迟高于单路。
...
```text
员工手册问题：
回答“周二、周四、前一个工作日 18:00 前”，
首条引用为正确 chunk，sources 同时包含
REDIS_VECTOR 与 ELASTICSEARCH_BM25。

差旅制度问题：
回答“一线城市 650 元/晚、10 个工作日内报销”，
引用覆盖 PDF 两个相关 chunk。

P1 故障问题：
回答“15 分钟内首次响应、60 分钟未恢复通知总经理和信息安全负责人”，
排名前两条引用均来自正确 TXT 文档。

知识缺失问题：
股票期权问题明确回答知识库未找到，没有编造年限和比例。
```

Day 6 交付物：

- 根目录 `readme.md`：当前能力、Mermaid 架构图、评测表、启动方式、五分钟 Demo 和已知边界。
- `docs/02-architecture-and-call-chain.md`：当前入库、Redis、BM25、RRF、MySQL 补全、RAG、Agent 和权限调用链。
- `docs/04-interview-guide.md`：混合格式上传证据、45 秒介绍、两分钟主线、简历描述、ownership、Demo 脚本和面试追问。
- `reports/interview-pack/`：按实习项目技能生成并补全的投递材料包。

材料验收：

```text
README / docs / interview-pack 本地 Markdown 链接检查通过；
不存在 TODO、旧版“尚未接入 Elasticsearch / 还不是 Agent”等过期表述；
简历只使用已经实现和验证的指标；
逐文档 ACL、Reranker、MCP、多 Agent、生产级死锁重试明确列为未实现。
```

本次真实批量上传还暴露一次 MySQL 死锁；失败文档通过单文档串行 vectorize 成功生成 3 个 chunk。复合问题还可能产生重复工具调用和重复引用。这些作为真实边界和后续方向记录，没有写成已优化成果。

因此第 6 天验收目标已经完成，六天核心计划总进度为 100%。
