# NEXUS Enterprise Knowledge Agent

基于 Java 21、Spring Boot、LangChain4j、Redis Stack、Elasticsearch 和 MySQL 的企业知识库 RAG 智能 Agent 项目。
...
- 支持 PDF、Word、TXT、Markdown 等企业文档解析、分块和异步入库；上传后通过 RocketMQ 消息驱动，Consumer 幂等消费并跟踪入库进度，失败自动重试，超限后进入 DLQ 并支持人工 retry。
...
- LangChain4j Agent 可自主决定是否调用 `searchKnowledgeBase`；Agent 回答前通过 LLM-as-a-Judge（`DraftJudgeService`）判断问题是否需要知识库，避免企业事实问题在未召回时被 LLM 直接裸答。
...
docker compose -f docker/docker-compose.yml up -d mysql redis elasticsearch namesrv broker
...
| LLM-as-a-Judge | `DraftJudgeService.judge()` |
| 文档解析 | `FileParseService.parseFile()` |
| 异步入库（MQ Producer） | `DocumentIngestionProducer.send()` |
| 异步入库（MQ Consumer） | `DocumentIngestionConsumer.onMessage()` |
| 入库幂等处理 | `DocumentIngestionService.process()` |
| 人工重试 | `POST /api/ingestion/tasks/{uploadId}/retry` |
